...
export default function VolleyballMode({ state }: VolleyballModeProps) {
  ...
  return (
    <div className="grid h-full w-full grid-rows-[16%_58%_26%] overflow-hidden bg-black px-2 py-1 font-mono text-white">
      ...
      <div className="grid min-h-0 grid-cols-[1fr_auto_1fr] items-center gap-1 overflow-hidden leading-none">
        <TeamScore label="HOME" value={homeScore} className="text-red-500" />
        <span className="text-[min(18cqh,12cqw)] font-black text-gray-700">-</span>
      ...
      <div className="grid min-h-0 grid-cols-[1fr_auto_1fr] items-center gap-1 overflow-hidden border-t border-gray-900 leading-none">
        <SetScore label="SETS" value={homeSets} className="text-red-400" />
        <span className="text-[min(8cqh,6cqw)] font-black text-gray-700">-</span>
        <SetScore label="SETS" value={awaySets} className="text-blue-400" />
      </div>
    </div>
  );
}
...
function TeamScore({ label, value, className }: { label: string; value: number; className: string }) {
  return (
    <div className="grid min-h-0 grid-rows-[20%_80%] overflow-hidden">
      <div className={`flex items-center justify-center text-[min(6cqh,5cqw)] font-black ${className}`}>{label}</div>
      <div className={`flex items-center justify-center text-[min(42cqh,34cqw)] font-black tabular-nums ${className}`}>{value}</div>
    </div>
  );
}
...
function SetScore({ label, value, className }: { label: string; value: number; className: string }) {
  return (
    <div className="grid min-h-0 grid-cols-[1fr_auto] items-center gap-1 overflow-hidden">
      <span className={`text-right text-[min(5cqh,4cqw)] font-black ${className}`}>{label}</span>
      <span className={`text-[min(14cqh,10cqw)] font-black tabular-nums ${className}`}>{value}</span>
    </div>
  );
}
